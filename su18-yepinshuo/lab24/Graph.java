...
public class Graph implements Iterable<Integer> {
    ...
    public List<Integer> shortestPath(int start, int stop) {
        PriorityQueue<Vertex>  fringe  = new PriorityQueue<>((x, y) -> x.distance - y.distance);
            ...
            for (Integer i : neighbors(v.id)) {
                ...
                if (!visited.contains(w)) {
                    if (!fringe.contains(w)) {
                        fringe.add(new Vertex(i,
                                v.distance + getEdge(v.id, i).weight, v));
                    } else {
                        if (v.distance + getEdge(v.id, i).weight < w.distance) {
                            w.distance = v.distance + getEdge(v.id, i).weight;
                            w.predecessor = v;
                            fringe.add(w);
                        }
                    }
    ...
    public class Vertex {
        ...
        public Vertex(int id, int distance, Vertex predecessor) {
            this.id = id;
