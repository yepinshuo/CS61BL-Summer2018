...
public class Graph {
    ...
    public Graph prims(int start) {
        ...
        PriorityQueue<Vertex> fringe =
                new PriorityQueue<>((x, y) -> x.distFromTree - y.distFromTree);
        fringe.add(new Vertex(start, 0, null));

        while (!fringe.isEmpty()) {
            Vertex v = fringe.poll();
            for (Integer i : getNeighbors(v.id)) {
                Vertex w = null;
                Edge x = null;
                for (Vertex vertex : fringe) {
                    if (vertex.id == i) {
                        w = vertex;
                        break;
                    }
                ...
                if (!T.containsVertex(i)) {
                    if (!fringe.contains(w)) {
                        fringe.add(new Vertex(i, x.getWeight(), x));
                    } else {
                        if (x.getWeight() < w.distFromTree) {
                            w.predecessor = x;
                            w.distFromTree = x.getWeight();
                            fringe.add(w);
                        }
                    }
                }
    ...
    public class Vertex {
        ...
        public Vertex(int id, int distance, Edge predecessor) {
            this.id = id;
            this.distFromTree = distance;
    ...
    public Graph kruskals() {
        ...
        for (int vertex : this.getAllVertices()) {
            result.addVertex(vertex);
        }
        TreeSet<Edge> unsortedEdge = this.getAllEdges();
        TreeSet<Edge> sortedEdge = new TreeSet<Edge>((o1, o2) -> o1.compareTo(o2));
        sortedEdge.addAll(unsortedEdge);
        for (Edge edge : sortedEdge) {
            if (!checkConnected(result, edge.getSource(), edge.getDest())) {
                result.addEdge(edge);
            }
        }
        return result;
    ...
    public boolean checkConnected(Graph graph, int v1, int v2) {
        ...
        while (!lst.isEmpty()) {
            ...
            if (!visited.contains(v)) {
                TreeSet<Integer> tree = graph.getNeighbors(v);
                if (tree.contains(v2)) {
                    return true;
                }
                for (int neighbor : tree) {
    ...
    public static void main(String[] args) {
        loadFromText("./inputs/graphTestNormal.in");
    }
}
